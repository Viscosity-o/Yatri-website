...
const useMap = lazy(() => import('react-leaflet').then(module => ({ default: module.useMap })));
...
function MapUpdater({ tourists, pois }: { tourists: TouristLocation[]; pois: PointOfInterest[] }) {
  const [map, setMap] = useState<any>(null);

  useEffect(() => {
    if (map && tourists.length > 0 && L) {
      const bounds = new L.LatLngBounds(tourists.map((t: any) => t.coordinates));
      if (pois.length > 0) {
        pois.forEach((poi: any) => bounds.extend(poi.coordinates));
      }
      ...
      <div className="relative" style={{ height }}>
        <Suspense fallback={<MapLoader height={height} />}>
          ...
          >
            ...
            <MapUpdater tourists={tourists} pois={pois} />
